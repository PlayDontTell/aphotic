...
const SCENES = {
	...
	"hangar_sas": "res://Chapters/Chapter_1/Scenes/HangarSas/hangar_sas.tscn",
}
...
func change_room(room, requestedPosition = Data.data.player.position,
		requestedRdvPoint = "null", requestedFlipH = Data.data.player.flip_h):
	...
	if has_node(room + "/Player"):
		...
		if requestedRdvPoint == "null":
			...
			var rdvPoint = get_node(room).rdvPoints[requestedRdvPoint]
			player.get_node("PlayerSprite").position = rdvPoint
